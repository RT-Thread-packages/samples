...
static int rt_hw_spi1_init(void)
{
    /* register spi bus */
    {
		...
		res = stm32_spi_bus_register(SPI1, SPI_BUS_NAME);
        if (res != RT_EOK)
		{
            OLED_TRACE("stm32_spi_bus_register!\r\n"); 
			return res;
		}
    }
    ...
    /* attach cs */
    {
        ...
        res = rt_spi_bus_attach_device(&spi_device, SPI_SSD1351_DEVICE_NAME, SPI_BUS_NAME, (void*)&spi_cs);
		if (res != RT_EOK)
		{
...
{
    struct rt_device                oled_device;
    struct rt_spi_device           *spi_device;
    struct rt_mutex                 lock;
    void *                          user_data;
};
static struct spi_oled_device  spi_oled_device;

static void ssd1351_lock(struct spi_oled_device * oled_device)
{
    rt_mutex_take(&oled_device->lock, RT_WAITING_FOREVER);
}

static void ssd1351_unlock(struct spi_oled_device * oled_device)
{
    rt_mutex_release(&oled_device->lock);
}
...
rt_err_t ssd1351_write_cmd(const rt_uint8_t cmd)
{
    ...
    //ssd1351_lock(&spi_oled_device);
    
    //len = rt_spi_send(spi_oled_device.spi_device, &cmd, 1);
    len = rt_spi_transfer(spi_oled_device.spi_device,  &cmd,  RT_NULL,  1);   
    
    //ssd1351_unlock(&spi_oled_device);
...
rt_err_t ssd1351_write_data(const rt_uint8_t data)
{
    ...
    //len = rt_spi_send(spi_oled_device.spi_device, &data, 1);
    len = rt_spi_transfer(spi_oled_device.spi_device,  &data,  RT_NULL,  1);
...
static rt_err_t ssd1351_oled_init(rt_device_t dev)
{
    return RT_EOK;
}

static rt_err_t ssd1351_oled_open(rt_device_t dev, rt_uint16_t oflag)
{
...
static rt_err_t ssd1351_oled_close(rt_device_t dev)
{
    return RT_EOK;
}

static rt_size_t ssd1351_oled_read(rt_device_t dev,
                                   rt_off_t pos,
                                   void* buffer,
                                   rt_size_t size)
{
...
static rt_size_t ssd1351_oled_write(rt_device_t dev,
                                    rt_off_t pos,
                                    const void* buffer,
                                    rt_size_t size)
{
...
static rt_err_t ssd1351_oled_control(rt_device_t dev, int cmd, void *args)
{
...
rt_err_t rt_hw_oled_init(const char * oled_device_name, const char * spi_device_name)
{
    
    struct rt_spi_device * rt_spi_device;
    
        /* initialize mutex */
    if (rt_mutex_init(&spi_oled_device.lock, spi_device_name, RT_IPC_FLAG_FIFO) != RT_EOK)
    {
        OLED_TRACE("init spi oled lock mutex failed\n");
        return -RT_ENOSYS;
    }
    ...
    {
        OLED_TRACE("spi device %s not found!\r\n", spi_device_name);
        return -RT_ENOSYS;
    }
    
    spi_oled_device.spi_device = rt_spi_device;
    ...
    /* config spi */
    {
        ...
        rt_spi_configure(spi_oled_device.spi_device, &cfg);
    }
    
    /* register Graphic device */
    spi_oled_device.oled_device.type    = RT_Device_Class_Graphic;
    spi_oled_device.oled_device.init    = ssd1351_oled_init;
    spi_oled_device.oled_device.open    = ssd1351_oled_open;
    spi_oled_device.oled_device.close   = ssd1351_oled_close;
    spi_oled_device.oled_device.read    = ssd1351_oled_read;
    spi_oled_device.oled_device.write   = ssd1351_oled_write;
    spi_oled_device.oled_device.control = ssd1351_oled_control;

    /* no private */
    spi_oled_device.oled_device.user_data = RT_NULL;

    rt_device_register(&spi_oled_device.oled_device, oled_device_name,
...
{ 
    ...
    rt_hw_oled_init(SPI_OLED_DEVICE_NAME, SPI_SSD1351_DEVICE_NAME);
    
    rt_hw_ssd1351_init();
    
    return 0;
}
INIT_PREV_EXPORT(rt_hw_spi_oled_init)
...
//        
//        // All Pixels On 
//        ssd1351_fill_ram(0xFFFF);
//        rt_thread_delay(500); 
//        
//        // Show Pattern - Format
//        //Show_64k_Pattern(&Format[0][0],0x10,0x6F,0x20,0x5F);
//        //rt_thread_delay(500); 
